...
def get_max_row_count_ws(current_worksheet,check_column='C',header_present=True):
    ivar = (2 if header_present else 1)
    while True:
        if current_worksheet[check_column+str(ivar)].value not in (None,'None',''):
            ivar+=1
        else:
            break
    return ivar-1
...
def addNewInvoice(modify=False,reset=False):
    ...
    if len(pastInvoices) != 0:
        ...
        currInvDate.set(invoiceNumDateDict[pastInvoices[-1][:-1]+str(int(pastInvoices[-1][-1]))])
    ...
    label_18.config(text='Invoice Date (dd/mm/yy):')
